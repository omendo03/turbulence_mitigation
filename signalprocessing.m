% Figure 5 (BER modified. Not present in original document)
...
for g = 1:columns(g_values)
  for u = 1:length(Pt)
     ...
     % SNR calculations with MIMO-STBC (Alamouti) for turbulence mitigation
     snr_noma(g, u) = 2 * pt(u) * a_values(g) * g_values(g) / no; % SNR for MIMO-NOMA with MIMO-NOMA (Alamouti)
     snr_oma(g, u) = 2 * pt(u) * g_values(g) / no; % SNR for MIMO-OMA with MIMO-NOMA (Alamouti)
...
title('SNR with Signal Processingfor turbulence mitigation');
...
title('BER with Signal Processing for turbulence mitigation');
